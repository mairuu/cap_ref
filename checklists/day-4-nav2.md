...
- [x] Recovery behaviours fire when you block it **and the spin one actually
      completes** — ✅ **CLOSED 16 Sep: `/spin` SUCCEEDED in 16.400653 s**,
      1.5762 rad against 1.57, inside the predicted 15.7–16.5 s. D-21's
      `time_allowance="25.0"` is proven loaded and used, and the stiction risk it
      flagged is disproven at the same `max_rotational_vel: 0.1`. (Two earlier
      attempts that day aborted at 25 s on an **unseated battery**, not friction
      — see the retraction in `records/calibration.md`.) Original note follows: — attempt 1 (15 Sep) failed here too, separately: upstream's
